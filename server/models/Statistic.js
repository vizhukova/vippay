     ...
     get(client_id){
        return new Promise((resolve, reject) => {
             ...
             knex.select(knex.raw(`
              ...
              (SELECT SUM( convertToBaseCurrency((orders.total_price_order_rate)::NUMERIC, ${client_id}, (orders.product->>'currency_id')::INTEGER) ) AS sum_pending_order FROM orders WHERE step = 'pending'),
              (SELECT SUM( convertToBaseCurrency((orders.total_price_order_rate)::NUMERIC, ${client_id}, (orders.product->>'currency_id')::INTEGER) ) AS sum_complete_order FROM orders WHERE step = 'complete'),
    ...
    getByPartner(data){
        return new Promise((resolve, reject) => {
             ...
             knex.select(knex.raw(`
              ...
              (SELECT SUM( convertToBaseCurrency((orders.total_price_order_rate)::NUMERIC, ${data.client_id}, (orders.product->>'currency_id')::INTEGER) ) AS sum_pending_order FROM orders WHERE step = 'pending' AND partner_id = ${data.partner_id}),
              (SELECT SUM( convertToBaseCurrency((orders.total_price_order_rate)::NUMERIC, ${data.client_id}, (orders.product->>'currency_id')::INTEGER) ) AS sum_complete_order FROM orders WHERE step = 'complete' AND partner_id = ${data.partner_id}),
